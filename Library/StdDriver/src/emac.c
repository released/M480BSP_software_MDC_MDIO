...
extern uint16_t software_MDIO_ReadRegister(uint8_t phyAddr, uint8_t regAddr);
extern void software_MDIO_WriteRegister(uint8_t phyAddr, uint8_t regAddr, uint16_t wdata);
...
{
    #if 0
    software_MDIO_WriteRegister(u32Reg,u32Addr,u32Data);
    #else
...
{
    #if 0
    return software_MDIO_ReadRegister(u32Reg,u32Addr);
    #else
    /* From preamble to idle is 64-bit transfer, MDC shouldn't be slower than 1MHz */
    uint32_t u32Delay = SystemCoreClock / 1000000 * 64;
